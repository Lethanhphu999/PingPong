...
{
	SDL_SetRenderDrawColor(Graphics::GetRenderer(), color.r, color.g, color.b, a); //tạo màu cho khối paddle
...
void Shape2D::RenderRect(uint8_t r, uint8_t g, uint8_t b, uint8_t a, bool renderFlag)
{
	if (renderFlag) {
		...
		SDL_SetRenderDrawColor(Graphics::GetRenderer(), r, g, b, a); //tạo màu cho khối paddle
...
void Shape2D::RenderCircle(uint8_t r, uint8_t g, uint8_t b, uint8_t a, bool renderFlag) {
	...
	if (renderFlag) {
		...
		SDL_SetRenderDrawColor(Graphics::GetRenderer(), r, g, b, a); //tạo màu cho trái bóng

		for (int y = 0; y < m_Circle.radius; ++y)
		{
			float x_point = sqrtf((m_Circle.radius * m_Circle.radius) - (y * y));

			SDL_RenderDrawLineF(Graphics::GetRenderer(), m_Circle.x - x_point, m_Circle.y - y, m_Circle.x + x_point, m_Circle.y - y); //vẽ nữa trên
			SDL_RenderDrawLineF(Graphics::GetRenderer(), m_Circle.x - x_point, m_Circle.y + y, m_Circle.x + x_point, m_Circle.y + y); //vẽ nửa dưới
